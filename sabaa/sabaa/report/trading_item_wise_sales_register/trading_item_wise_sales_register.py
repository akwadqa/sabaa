...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
    ...
    for d in item_list:
        ...
        row["qty_by_uom"] = get_qty_by_uom_display(
            d.item_code,
            d.stock_qty,
            d.stock_uom,
            uom_conversion_cache
...
def get_qty_by_uom_display(item_code, stock_qty, stock_uom, cache):
    """
    Convert stock_qty to multi-UOM display safely.
    Handles:
    - negative values
    - missing conversions
    - fractional factors
    ...
    if item_code not in cache:
        ...
        # fallback if item has no conversion rows
        if not conversion_map:
            conversion_map = {stock_uom: 1}

        cache[item_code] = conversion_map

    uom_map = cache[item_code]

    sign = -1 if stock_qty < 0 else 1
    qty = abs(stock_qty)

    qty_str = format_quantity_by_uoms(qty, uom_map)
