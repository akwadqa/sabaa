...
def _execute(filters, additional_table_columns=None):
    ...
    for inv in invoice_list:
        ...
        row = {
            "voucher_no": f"<a href='{frappe.utils.get_url_to_form(inv.doctype, inv.name)}' target='_blank'>{inv.name}</a>",
            "posting_date": inv.posting_date,
            "customer_name": f"<a href='{frappe.utils.get_url_to_form('Customer', inv.customer)}' target='_blank'>{inv.customer_name}</a>",
...
def get_descendants_of_customer_group(customer_group):
    ...
    # Include the parent group itself
...
def get_invoices(filters, additional_query_columns):
    ...
    if not filters.get("include_returns"):
        query = query.where((si.is_return.isnull()) | (si.is_return == 0))
