...
def _execute(filters=None, additional_table_columns=None):
	...
	paid_amount_map = get_paid_amount_map(
		[inv.name for inv in invoice_list if inv.doctype == "Purchase Invoice"]
	)
	...
	for inv in invoice_list:
		...
		# total tax, grand total, rounded total, paid amount & outstanding amount
		row.update(
			{
				"total_tax": total_tax,
				"grand_total": inv.base_grand_total,
				"rounded_total": inv.base_rounded_total,
				"paid_amount": flt(paid_amount_map.get(inv.name)) if inv.doctype == "Purchase Invoice" else None,
...
def get_paid_amount_map(invoice_names):
	"""Paid Amount per Purchase Invoice = linked Payment Entry allocations + linked Journal Entry debits."""
	paid_amount_map = defaultdict(float)
	...
	payment_rows = (
		...
		.select(per.reference_name, Sum(per.allocated_amount).as_("amount"))
	...
	for row in payment_rows:
		paid_amount_map[row.reference_name] += flt(row.amount)
	...
	journal_rows = (
		...
		.select(jea.reference_name, Sum(jea.debit).as_("amount"))
