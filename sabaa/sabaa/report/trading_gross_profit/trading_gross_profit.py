...
def get_data_when_grouped_by_invoice(columns, gross_profit_data, filters, group_wise_columns, data):
    ...
    # Remove unwanted columns for Invoice grouping
    remove_fields = [
        "customer_group",
        "item_group",
        "brand",
    ...
    columns[:] = [
        ...
        if not (isinstance(col, dict) and col.get("fieldname") in remove_fields)
    ]
...
class GrossProfitGenerator:
    ...
    def load_invoice_items(self):
        ...
        invoice_type = self.filters.get("invoice_type")

        if invoice_type == "Sales Invoice":
            conditions += " and is_return = 0"

        elif invoice_type == "Return Invoice":
            conditions += " and is_return = 1"

        else:  # "All" invoices
            if self.filters.get("include_returned_invoices"):
                # Include returned invoices that are not linked to a return_against
                conditions += " and (is_return = 0 or (is_return=1 and return_against is null))"
            else:
                conditions += " and is_return = 0"
        
        if self.filters.get("customer_group"):
            from erpnext.accounts.doctype.accounting_dimension.accounting_dimension import get_dimension_with_children

            all_customer_groups = get_dimension_with_children("Customer Group", self.filters.get("customer_group"))
            self.filters.customer_group = all_customer_groups

            conditions += " and customer_group in %(customer_group)s"
